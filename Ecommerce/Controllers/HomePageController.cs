...
namespace Ecommerce.Controllers
{
	public class HomePageController : Controller
	{
        ...
        public async Task<IActionResult> Home()
        {
			...
			string cookieContent = Request.Cookies["S#$51%^Lm*A!2@m"];

			if (!String.IsNullOrWhiteSpace(cookieContent))
			{
				HttpContext.Session.SetInt32("CartItemsCount", cookieContent.Split(',', StringSplitOptions.RemoveEmptyEntries).Length);
			}
		...
		public IEnumerable<ProductFieldViewModel> GetProductFilds(int id)
		{
			...
			var productFieldViewModels = model.ToList();
			var selectIdProductField = productFieldViewModels.Where(x => x.Type == 4).FirstOrDefault();
			...
			try
			{
				var item = productFieldViewModels.FirstOrDefault();
				if (item != null)
				{
					item.CarListItems = _context.Cars.Select(c => new SelectListItem
					{
						Text = c.Name,
						Value = c.Id.ToString()
					}).ToList();
			...
			using (var db = _iserviceProvider.GetRequiredService<ApplicationDbContext>())
			{
				...
				else
				{
					...
					if (Request.Cookies["S#$51%^Lm*A!2@m"] == null)
					{
						...
						HttpContext.Session.SetInt32("CartItemsCount", 1);
					...
					else
					{
						...
						if (cookicontent.Contains("," + productid + ","))
						{
							...
							HttpContext.Session.SetInt32("CartItemsCount", requestbookcount.Length);

							return Json(new { status = "success", message = "این محصول از قبل در لیست درخواستی شما وجود دارد", sabadcount = requestbookcount.Length });
						}
						else
						{
