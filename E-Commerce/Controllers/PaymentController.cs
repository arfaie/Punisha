			...
			using (_context)
			{
				using (var transaction = _context.Database.BeginTransaction())
				{
					try
					{
						...
						{
							p.FactorId = HttpContext.Session.GetString("F#%&I!^8@6od");

							_context.Orders.Add(p);

							var selectedFactor = await _context.Factors.SingleOrDefaultAsync(f => f.Id == p.FactorId);

							selectedFactor.IsPaid = true;
